...
#define XAE_AF2_OFFSET		0x0000071C /* Address Filter 2 */
...
{
	return in_be32(lp->qbv_regs + offset);
}
...
static inline void axienet_qbv_iow(struct axienet_local *lp, off_t offset,
				   u32 value)
{
	out_be32((lp->qbv_regs + offset), value);
}
#endif
...
int axienet_tsn_probe(struct platform_device *pdev,
		      ...
		      struct net_device *ndev);
int tsn_mcdma_probe(struct platform_device *pdev,
		  struct axienet_local *lp,
		  struct net_device *ndev);
...
int axienet_tadma_probe(struct platform_device *pdev,
			struct net_device *ndev);
