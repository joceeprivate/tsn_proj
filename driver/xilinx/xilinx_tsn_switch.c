...
u8 en_hw_addr_learning;
u8 inband_mgmt_tag;
u8 sw_mac_addr[ETH_ALEN];
...
int tsn_switch_cam_set(struct cam_struct data, u8 add)
{
	...
	if (add && (data.fwd_port & PORT_EX_ONLY) ||
		(data.fwd_port & PORT_EX_EP)) {
		if (!(ep_lp->ex_ep)) {
			pr_err("Endpoint extension support is not present"
				" in this design\n");
			return -EINVAL;
		} else if ((data.fwd_port & PORT_EX_ONLY) && 
				(data.fwd_port & PORT_EX_EP)) {
			if (!(ep_lp->packet_switch)) {
				pr_err("Support for forwarding packets from"
					" endpoint to extended endpoint or"
					" vice versa is not present in this"
					" design\n");
				return -EINVAL;
			}
		}
	...
	/* wmb to fix HW hang issue while reading KEY2 and TV1 register
	 * sequentially
	 */
	wmb();
	/* TV 1 and TV 2 */
...
static int read_cam_entry(struct cam_struct data, void __user *arg)
{
	...
	timeout = 20000;
	while (((u_value = axienet_ior(&lp, XAS_SDL_CAM_CTRL_OFFSET)) &
				SDL_CAM_WR_ENABLE) && timeout)
		timeout--;
	if (!timeout)
		pr_warn("CAM write took longer time !!");
...
static int get_mac_addr_learnt_list(void __user *arg)
{
	...
	/* wait for cam init done */
	while (!((u_value = axienet_ior(&lp, XAS_SDL_CAM_STATUS_OFFSET)) &
		SDL_CAM_WR_ENABLE) && timeout)
		timeout--;
	if (!timeout)
		pr_warn("CAM init took longer time!!");
	...
	for (i = 0; i < MAX_NUM_MAC_ENTRIES ; i++) {
		...
		while (!((u_value = axienet_ior(&lp, XAS_SDL_CAM_STATUS_OFFSET))
			...
			timeout--;
		if (!timeout)
			pr_warn("CAM init took longer time %d!!", i);
		...
		while (((u_value = axienet_ior(&lp, XAS_SDL_CAM_CTRL_OFFSET)) &
			...
			timeout--;
		if (!timeout)
			pr_warn("CAM write took longer time %d!!", i);
...
int tsn_switch_set_stp_state(struct port_status *port)
{
	...
	/* wait for write to complete */
	while (((u_value = axienet_ior(&lp, XAS_PORT_STATE_CTRL_OFFSET)) &
		en_port_sts_chg_bit) && timeout)
		timeout--;
	if (!timeout)
		pr_warn("CAM write took longer time!!");
	return 0;
...
static int get_port_status(void __user *arg)
{
	...
	case PORT_EX_ONLY:
		if (ep_lp->ex_ep) {
			port.port_status = (u_value >> EX_EP_PORT_STATUS_SHIFT) &
						PORT_STATUS_MASK;
			break;
		}
		else {
...
int tsn_switch_vlan_add(struct port_vlan *port, int add)
{
	...
	if(learning) {
		if (port->en_ipv == 1) {
			pr_err("When hardware address learning is enabled"
	" for a VLAN, TSN streams cannot be mapped to a particular"
	" priority queue\n");
			return -EPERM;
		}
	} else {
		if (port->en_port_status == 1) {
			pr_err("When hardware address learning is disabled"
	"for a VLAN, port status cannot be set per VLAN\n");
			return -EPERM;
		}
	}
	...
	while ((axienet_ior(&lp, XAS_VLAN_MEMB_CTRL_REG) &
		PORT_VLAN_WRITE_READ_EN_BIT) && timeout)
		timeout--;
	if (!timeout)
		pr_warn("Port vlan write took longer time!!");
...
static int set_vlan_mac_addr_learn(void __user *arg)
{
	...
	/* wait for port vlan write completion */
	while (((u_value = axienet_ior(&lp, XAS_VLAN_MEMB_CTRL_REG)) &
		PORT_VLAN_WRITE_READ_EN_BIT) && timeout)
		timeout--;
	if (!timeout)
		pr_warn("Port vlan write took longer time!!");
...
static int get_vlan_mac_addr_learn(void __user *arg)
{
	...
	/* wait for port vlan write completion */
	while (((u_value1 = axienet_ior(&lp, XAS_VLAN_MEMB_CTRL_REG)) &
		PORT_VLAN_WRITE_READ_EN_BIT) && timeout)
		timeout--;
	if (!timeout)
		pr_warn("Port vlan write took longer time!!");
			 ...
			 unsigned long arg)
{
	...
	case READ_CAM_ENTRY:
		...
		if (read_cam_entry(data.cam_data, (void __user *)arg)) {
			retval = -EINVAL;
			goto end;
		}
		break;
	...
	case GET_PORT_STATUS:
		...
		break;
	case ADD_PORT_VLAN:
		if(add_del_port_vlan((void __user *)arg, ADD) < 0)
			return -EPERM;
		break;
	case DEL_PORT_VLAN:
		if(add_del_port_vlan((void __user *)arg, DELETE) < 0)
			return -EPERM;
		break;
...
static int tsn_switch_fdb_init(struct platform_device *pdev)
{
	...
	cam.fwd_port = 7; //(PORT_EP | PORT_MAC1 | PORT_MAC2);
	cam.flags |= XAS_CAM_EP_MGMTQ_EN;
	cam.vlanid = 1; /* default pvid */
...
static int tsnswitch_probe(struct platform_device *pdev)
{
	...
	/* writting into endpoint extention control register for channel mapping as follows:
	 *
	...
	if(ep_lp->ex_ep) {
		 ...
		 * and multicast packets received from endpoint
		 */
