...
irqreturn_t __maybe_unused axienet_mcdma_rx_irq(int irq, void *_ndev)
{
	...
	/* DMA Normal rx irq handling. Using new NAPI. */
	if (status & (XMCDMA_IRQ_IOC_MASK | XMCDMA_IRQ_DELAY_MASK)) {
		cr = axienet_dma_in32(q, XMCDMA_CHAN_CR_OFFSET(q->chan_id) +
				      q->rx_offset);
	...
	/* DMA error irq handling. Using old netif_rx with tasklet. */
	if (status & XMCDMA_IRQ_ERR_MASK) {
	...
	for (i = 0, j = 0; i < AXIENET_TX_SSTATS_LEN(lp);) {
		...
		if (q == NULL) {
			if (sset == ETH_SS_STATS) {
				memcpy(data + l++ * ETH_GSTRING_LEN,
					"tx_packets", ETH_GSTRING_LEN);
				memcpy(data + l++ * ETH_GSTRING_LEN,
					"tx_bytes", ETH_GSTRING_LEN);
				memcpy(data + l++ * ETH_GSTRING_LEN,
					"rx_packets", ETH_GSTRING_LEN);
				memcpy(data + l++ * ETH_GSTRING_LEN,
					"rx_bytes", ETH_GSTRING_LEN);
				}
			return;
		}
		if (i % 2 == 0)
					  ...
					  struct axienet_local *lp)
{
	...
	of_property_read_u32(np, "xlnx,num-mm2s-channels", &num);
