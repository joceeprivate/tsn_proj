...
void change_filter_values_to_udp(struct axienet_local *lp)
{
	axienet_iow(lp, 0x708, 0x100);
	/**
	 * axienet_iow(lp, 0x70C, 0x0); values may not
	 * be written on to the specified address if this is not given
	 */
	axienet_iow(lp, XAE_AF2_OFFSET, 0x00000008);
	axienet_iow(lp, 0x724, 0x11000000);
	axienet_iow(lp, 0x734, 0x00003f01);
	axienet_iow(lp, 0x738, 0x02000000);

	axienet_iow(lp, 0x750, 0x0);
	axienet_iow(lp, 0x754, 0x0);
	axienet_iow(lp, 0x764, 0xff000000);
	axienet_iow(lp, 0x774, 0x0000ffff);
	axienet_iow(lp, 0x778, 0xff000000);
}

void change_filter_values_to_gptp(struct axienet_local *lp)
{
	axienet_iow(lp, XAE_AF2_OFFSET, 0x0000F788);
	axienet_iow(lp, 0x750, 0xffffffff);
	axienet_iow(lp, 0x754, 0x0000ffff);
	axienet_iow(lp, 0x764, 0x0);
	axienet_iow(lp, 0x774, 0x0);
	axienet_iow(lp, 0x778, 0x0);
}
				      ...
				      struct ethtool_regs *regs, void *ret)
{
	...
	memset(data, 0, len);
	if (!lp->is_tsn) {
		//The following registers are reserved in case of TSN configuration
					      ...
					      struct net_device *ndev)
{
	...
	axienet_mcdma_rx_probe(pdev, np, ndev);
