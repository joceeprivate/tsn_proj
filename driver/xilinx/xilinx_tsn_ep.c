...
MODULE_PARM_DESC(st_pcp, "Array of pcp values mapped to ST class" 
			"at the compile time");
u8 res_pcp[8];
uint res_count = 0;
module_param_array(res_pcp, byte, &res_count,0644);
MODULE_PARM_DESC(res_pcp, "Array of pcp values mapped to RES class" 
			"at the compile time");
...
int tsn_data_path_open(struct net_device *ndev)
{
	...
	for_each_tx_dma_queue(lp, i) {
		q = lp->dq[i];
		/*MCDMA TX RESET*/
		__axienet_device_reset(q, XMCDMA_CR_OFFSET);
	}
 ...
 */
int __maybe_unused tsn_mcdma_probe(struct platform_device *pdev,
...
int axienet_get_pcp_mask(struct axienet_local *lp, u16 num_tc)
{
	...
	if (st_count == 0 || st_count > 8) {
		...
		if (invalid_pcp) {
			pr_warn("pcp value cannot be greater than or"
				"equal to 8\n");
			lp->st_pcp = 1 << 4;
			invalid_pcp = 0;
		}
	...
	if (num_tc == 3) {
		if (res_count == 0 || res_count > 8) {
			...
			if (invalid_pcp) {
				pr_warn("pcp value cannot be greater than"
					" or equal to 8\n");
				lp->res_pcp = 1 << 2 | 1 << 3;
			}
		}
...
static int tsn_ep_probe(struct platform_device *pdev)
{
	...
	/* SWITCHDEV ops */
#if defined(CONFIG_NET_SWITCHDEV) && defined(CONFIG_XILINX_TSN_SWITCH)
	xlnx_switchdev_ops_add(ndev);
#endif
	return ret;
