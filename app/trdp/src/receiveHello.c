...
int main(int argc, char *argv[])
{
    ...
    TRDP_SUB_T *subHandle = NULL;         /*    Our identifier to the publication         */
    ...
    while ((ch = getopt(argc, argv, "o:m:h?vc:n:")) != -1)
    {
        switch (ch)
        {
        case 'o':
    ...
    subHandle = (TRDP_SUB_T *) vos_memAlloc(processNum * sizeof(TRDP_SUB_T));
    memset(subHandle, 0, processNum * sizeof(TRDP_SUB_T));
    memset(gBuffer, 0, sizeof(gBuffer));    

    for (i = 0; i < processNum; i ++)
    {
        err = tlp_subscribe(appHandle,                      /*    our application identifier            */
                            subHandle + i,                     /*    our subscription identifier           */
                            NULL,                           /*    user reference                        */
    ...
    while (1)
    {
        ...
        /*
         Prevent from running too fast, if we're just waiting for packets (default min. time is 2ms).
        */
        ...
        */
        ...
        for (i = 0; i < processNum; i ++)
        {
            ...
            if ((TRDP_NO_ERR == err) && (receivedSize > 0))
            {
                ...
                vos_printLog(VOS_LOG_USR, "COMID: %d ", myPDInfo.comId);
