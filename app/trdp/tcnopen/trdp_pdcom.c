 ...
 *
 * $Id: trdp_pdcom.c 1836 2019-02-05 16:11:47Z bloehr $
 *
    ...
    INT32           sock)
{
    ...
    //printf("RECV : COMID = %d, SRC = %d, DST = %d\n", subAddresses.comId, subAddresses.srcIpAddr, subAddresses.destIpAddr);
    ...
    else
    {
                                   ...
                                   pExistingElement->addr.opTrnTopoCnt))
        {
            ...
            /* MODIFIED */
            //
            //vos_printLog(VOS_LOG_USR, "COMID = %d, %d | %d\n", subAddresses.comId, now.tv_sec, now.tv_usec);
            //vos_printLog(VOS_LOG_USR, "Recv: COMID = %d, seq = %d\n", subAddresses.comId, newSeqCnt);            
            // if (newSeqCnt && newSeqCnt != seqcnt[subAddresses.comId%128] + 1)
            // {
            //     vos_getTime(&now);
            //     printf("Dropped: %d, %d\n", now.tv_sec, now.tv_usec);
            //     printf("Dropped: COMID = %d, newseq = %d, oldseq = %d\n", subAddresses.comId, newSeqCnt, seqcnt[subAddresses.comId%128]);
            // }
            seqcnt[subAddresses.comId%128] = newSeqCnt;
    ...
    TRDP_SESSION_PT appHandle)
{
    ...
    for (iterPD = appHandle->pRcvQueue; iterPD != NULL; iterPD = iterPD->pNext)
    {
        ...
        /*    Update the current time    */
        /* (MODIFIED) Avoid getting time too often */
        //vos_getTime(&now);
    }
}
