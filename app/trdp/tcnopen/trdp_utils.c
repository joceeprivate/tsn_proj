 ...
 *
 * $Id: trdp_utils.c 1829 2019-01-17 09:38:36Z s-bender $
 *
    ...
    TRDP_ADDRESSES_T    *addr)
{
    ...
    for (iterPD = pHead; iterPD != NULL; iterPD = iterPD->pNext)
    {
        /*  We match if src/dst/mc address is zero or matches */
        if ((iterPD->addr.comId == addr->comId)
            && ((iterPD->addr.srcIpAddr == VOS_INADDR_ANY) || (iterPD->addr.srcIpAddr == addr->srcIpAddr))
            && ((iterPD->addr.destIpAddr == VOS_INADDR_ANY) || (iterPD->addr.destIpAddr == addr->destIpAddr)))		/*LSL:20190421	˫�������࣬��ȡcomID���¶���ʱҪУ��Ŀ��IP*/
        {
            return iterPD;
        }
    }
